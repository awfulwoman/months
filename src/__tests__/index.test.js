...
describe('Check parameter values', () => {
  ...
  test('Pass on strings that are months', () => {
    expect(months('january')).toEqual({ abbr: 'jan', name: 'january', number: 1 })
    expect(months('mar')).toEqual({ abbr: 'mar', name: 'march', number: 3 })
  })
  ...
  test('Pass with months that have same name and abbr', () => {
    expect(months('may')).toEqual({ abbr: 'may', name: 'may', number: 5 })
    expect(months('May')).toEqual({ abbr: 'may', name: 'may', number: 5 })
  })
})
